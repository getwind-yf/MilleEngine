...
void vkglTF::Texture::fromglTfImage(tinygltf::Image& gltfimage, std::string path, vent::Device* device, VkQueue copyQueue)
{
	this->device = device;
	...
	if (!isKtx) {
		...
		if (!asset) {
			vent::utils::exitFatal("Could not load texture from " + filename + "\n\nMake sure the assets submodule has been checked out and is up-to-date.", -1);
		}
		...
		delete[] textureData;
#else
		if (!vent::utils::fileExists(filename)) {
			vent::utils::exitFatal("Could not load texture from " + filename + "\n\nMake sure the assets submodule has been checked out and is up-to-date.", -1);
		}
		...
		VkBufferCreateInfo bufferCreateInfo = vent::vkinitializer::bufferCreateInfo();
		...
		VkImageCreateInfo imageCreateInfo = vent::vkinitializer::imageCreateInfo();
		imageCreateInfo.imageType = VK_IMAGE_TYPE_2D;
		imageCreateInfo.format = format;
		imageCreateInfo.mipLevels = mipLevels;
		imageCreateInfo.arrayLayers = 1;
		...
		vent::utils::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
		...
		vent::utils::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
...
/*
	glTF mesh
*/
vkglTF::Mesh::Mesh(vent::Device* device, glm::mat4 matrix) {
	this->device = device;
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
	...
	VkCommandBuffer copyCmd = device->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
	vent::utils::setImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
	vkCmdCopyBufferToImage(copyCmd, stagingBuffer, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &bufferCopyRegion);
	vent::utils::setImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
	...
	VkSamplerCreateInfo samplerCreateInfo = vent::vkinitializer::samplerCreateInfo();
	...
	VkImageViewCreateInfo viewCreateInfo = vent::vkinitializer::imageViewCreateInfo();
...
void vkglTF::Model::loadImages(tinygltf::Model& gltfModel, vent::Device* device, VkQueue transferQueue)
{
...
void vkglTF::Model::loadFromFile(std::string filename, vent::Device* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	if (fileLoaded) {
		...
		vent::utils::exitFatal("Could not load glTF file \"" + filename + "\": " + error, -1);
		return;
	}
	...
	// Descriptors for per-material images
	{
		...
		if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
			std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings{};
			if (descriptorBindingFlags & DescriptorBindingFlags::ImageBaseColor) {
				setLayoutBindings.push_back(vent::vkinitializer::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, static_cast<uint32_t>(setLayoutBindings.size())));
			}
			if (descriptorBindingFlags & DescriptorBindingFlags::ImageNormalMap) {
				setLayoutBindings.push_back(vent::vkinitializer::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, static_cast<uint32_t>(setLayoutBindings.size())));
			}
