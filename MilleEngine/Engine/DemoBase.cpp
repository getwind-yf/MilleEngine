...
std::vector<const char*> DemoBase::args;

VkResult DemoBase::createInstance()
{
	...
	if (settings.validation) {
		vent::debug::setupDebugingMessengerCreateInfo(debugUtilsMessengerCI);
	...
	if (std::find(supportedInstanceExtensions.begin(), supportedInstanceExtensions.end(), VK_EXT_DEBUG_UTILS_EXTENSION_NAME) != supportedInstanceExtensions.end()) {
		vent::debugutils::setup(instance);
	}

	return result;
}

void DemoBase::renderFrame()
{
	DemoBase::prepareFrame();
	submitInfo.commandBufferCount = 1;
	submitInfo.pCommandBuffers = &drawCmdBuffers[currentBuffer];
	VK_CHECK_RESULT(vkQueueSubmit(queue, 1, &submitInfo, VK_NULL_HANDLE));
	DemoBase::submitFrame();
}

std::string DemoBase::getWindowTitle()
{
...
void DemoBase::createCommandBuffers()
{
	...
	VkCommandBufferAllocateInfo cmdBufAllocateInfo = vent::vkinitializer::commandBufferAllocateInfo(cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, static_cast<uint32_t>(drawCmdBuffers.size()));
...
std::string DemoBase::getShadersPath() const
{
...
VkPipelineShaderStageCreateInfo DemoBase::loadShader(std::string fileName, VkShaderStageFlagBits stage)
{
	VkPipelineShaderStageCreateInfo shaderStage = {};
	shaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
	shaderStage.stage = stage;
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
	shaderStage.module = vent::utils::loadShader(androidApp->activity->assetManager, fileName.c_str(), device);
#else
	shaderStage.module = vent::utils::loadShader(fileName.c_str(), device);
#endif
...
void DemoBase::drawUI(const VkCommandBuffer commandBuffer)
{
	if (settings.overlay && ui.visible) {
		const VkViewport viewport = vent::vkinitializer::viewport((float)width, (float)height, 0.0f, 1.0f);
		const VkRect2D scissor = vent::vkinitializer::rect2D(width, height, 0, 0);
...
DemoBase::DemoBase()
{
	...
	if (commandLineParser.isSet("benchmark")) {
		...
		vent::utils::errorModeSilent = true;
	}
...
DemoBase::~DemoBase()
{
	...
	if (settings.validation)
	{
		vent::debug::freeDebugCallback(instance);
	}
...
bool DemoBase::initVulkan()
{
	...
	if (result != VK_SUCCESS) {
		vent::utils::exitFatal("Could not create Vulkan instance : \n" + vent::utils::errorString(result), result);
		return false;
	}
	...
	if (settings.validation)
	{
		vent::debug::setupDebugging(instance);
	}
	...
	if (gpuCount == 0) {
		vent::utils::exitFatal("No device with Vulkan support found", -1);
		return false;
	}
	// Enumerate devices
	...
	if (result != VK_SUCCESS) {
		vent::utils::exitFatal("Could not enumerate physical devices : \n" + vent::utils::errorString(result), result);
		return false;
	}
	...
	if (commandLineParser.isSet("gpulist")) {
		...
		for (uint32_t i = 0; i < gpuCount; i++) {
			...
			std::cout << " Type: " << vent::utils::physicalDeviceTypeString(deviceProperties.deviceType) << "\n";
	...
	vulkanDevice = new vent::Device(physicalDevice);
	...
	if (result != VK_SUCCESS) {
		vent::utils::exitFatal("Could not create Vulkan device: \n" + vent::utils::errorString(result), result);
		return false;
	}
	...
	if (requiresStencil) {
		validFormat = vent::utils::getSupportedDepthStencilFormat(physicalDevice, &depthFormat);
	}
	else {
		validFormat = vent::utils::getSupportedDepthFormat(physicalDevice, &depthFormat);
	}
	assert(validFormat);
	...
	VkSemaphoreCreateInfo semaphoreCreateInfo = vent::vkinitializer::semaphoreCreateInfo();
	...
	submitInfo = vent::vkinitializer::submitInfo();
...
void DemoBase::setupConsole(std::string title)
{
	AllocConsole();
...
void DemoBase::setupDPIAwareness()
{
...
HWND DemoBase::setupWindow(HINSTANCE hinstance, WNDPROC wndproc)
{
...
void DemoBase::handleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_CLOSE:
...
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
int32_t DemoBase::handleAppInput(struct android_app* app, AInputEvent* event)
{
	DemoBase* vulkanExample = reinterpret_cast<DemoBase*>(app->userData);
...
void DemoBase::handleAppCommand(android_app* app, int32_t cmd)
{
	...
	void* displayLinkContext)
{
	@autoreleasepool
	{
		auto vulkanExample = static_cast<DemoBase*>(displayLinkContext);
			vulkanExample->displayLinkOutputCb();
	}
...
// SRS - Window resizing already handled by windowResize() in DemoBase::submitFrame()
...
void* DemoBase::setupWindow(void* view)
{
#if defined(VK_EXAMPLE_XCODE_GENERATED)
...
void DemoBase::displayLinkOutputCb()
{
#if defined(VK_EXAMPLE_XCODE_GENERATED)
	if (benchmark.active) {
		benchmark.run([=] { render(); }, vulkanDevice->properties);
...
void DemoBase::mouseDragged(float x, float y)
{
	handleMouseMove(static_cast<uint32_t>(x), static_cast<uint32_t>(y));
}

void DemoBase::windowWillResize(float x, float y)
{
	resizing = true;
	if (prepared)
	{
...
void DemoBase::windowDidResize()
{
	resizing = false;
}
#elif defined(_DIRECT2DISPLAY)
#elif defined(VK_USE_PLATFORM_DIRECTFB_EXT)
IDirectFBSurface* DemoBase::setupWindow()
{
	DFBResult ret;
	int posx = 0, posy = 0;
...
void DemoBase::handleEvent(const DFBWindowEvent* event)
{
	switch (event->type)
	{
	case DWET_CLOSE:
...
#elif defined(VK_USE_PLATFORM_WAYLAND_KHR)
/*static*/void DemoBase::registryGlobalCb(void* data,
	wl_registry* registry, uint32_t name, const char* interface,
	uint32_t version)
{
	DemoBase* self = reinterpret_cast<DemoBase*>(data);
	self->registryGlobal(registry, name, interface, version);
}

/*static*/void DemoBase::seatCapabilitiesCb(void* data, wl_seat* seat,
	uint32_t caps)
{
	DemoBase* self = reinterpret_cast<DemoBase*>(data);
...
/*static*/void DemoBase::pointerEnterCb(void* data,
	wl_pointer* pointer, uint32_t serial, wl_surface* surface,
	wl_fixed_t sx, wl_fixed_t sy)
{
}

/*static*/void DemoBase::pointerLeaveCb(void* data,
	wl_pointer* pointer, uint32_t serial, wl_surface* surface)
{
}

/*static*/void DemoBase::pointerMotionCb(void* data,
	wl_pointer* pointer, uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
	DemoBase* self = reinterpret_cast<DemoBase*>(data);
	self->pointerMotion(pointer, time, sx, sy);
}
void DemoBase::pointerMotion(wl_pointer* pointer, uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
	handleMouseMove(wl_fixed_to_int(sx), wl_fixed_to_int(sy));
}

/*static*/void DemoBase::pointerButtonCb(void* data,
...
void DemoBase::pointerButton(struct wl_pointer* pointer,
...
/*static*/void DemoBase::pointerAxisCb(void* data,
...
void DemoBase::pointerAxis(wl_pointer* pointer, uint32_t time,
...
/*static*/void DemoBase::keyboardKeymapCb(void* data,
	struct wl_keyboard* keyboard, uint32_t format, int fd, uint32_t size)
{
}

/*static*/void DemoBase::keyboardEnterCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial,
	struct wl_surface* surface, struct wl_array* keys)
{
}

/*static*/void DemoBase::keyboardLeaveCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial,
	struct wl_surface* surface)
{
}

/*static*/void DemoBase::keyboardKeyCb(void* data,
...
void DemoBase::keyboardKey(struct wl_keyboard* keyboard,
...
/*static*/void DemoBase::keyboardModifiersCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial, uint32_t mods_depressed,
	uint32_t mods_latched, uint32_t mods_locked, uint32_t group)
{
}

void DemoBase::seatCapabilities(wl_seat* seat, uint32_t caps)
{
...
void DemoBase::registryGlobal(wl_registry* registry, uint32_t name,
...
/*static*/void DemoBase::registryGlobalRemoveCb(void* data,
	struct wl_registry* registry, uint32_t name)
{
}

void DemoBase::initWaylandConnection()
{
...
void DemoBase::setSize(int width, int height)
{
	if (width <= 0 || height <= 0)
		return;
...
struct xdg_surface* DemoBase::setupWindow()
{
...
xcb_window_t DemoBase::setupWindow()
{
...
void DemoBase::initxcbConnection()
{
...
void DemoBase::handleEvent(const xcb_generic_event_t* event)
{
...
void DemoBase::keyPressed(uint32_t) {}

void DemoBase::mouseMoved(double x, double y, bool& handled) {}

void DemoBase::buildCommandBuffers() {}

void DemoBase::createSynchronizationPrimitives()
{
	// Wait fences to sync command buffer access
	VkFenceCreateInfo fenceCreateInfo = vent::vkinitializer::fenceCreateInfo(VK_FENCE_CREATE_SIGNALED_BIT);
...
void DemoBase::createCommandPool()
{
...
void DemoBase::setupDepthStencil()
{
...
void DemoBase::setupFrameBuffer()
{
...
void DemoBase::getEnabledFeatures() {}

void DemoBase::getEnabledExtensions() {}

void DemoBase::windowResize()
{
	if (!prepared)
	{
		return;
...
void DemoBase::handleMouseMove(int32_t x, int32_t y)
{
...
void DemoBase::windowResized() {}

void DemoBase::initSwapchain()
{
#if defined(_WIN32)
...
void DemoBase::setupSwapChain()
{
	swapChain.create(&width, &height, settings.vsync, settings.fullscreen);
}

void DemoBase::OnUpdateUIOverlay(vent::UIWrapper* overlay) {}

#if defined(_WIN32)
void DemoBase::OnHandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {};
#endif
