...
class VulkanExample : public DemoBase
{
public:
	...
	*/
	void prepareDepthPass()
	{
		...
		*/
		...
		VkRenderPassCreateInfo renderPassCreateInfo = vent::vkinitializer::renderPassCreateInfo();
		...
		*/
		...
		for (uint32_t i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
			...
			// Framebuffer
			VkFramebufferCreateInfo framebufferInfo = vent::vkinitializer::framebufferCreateInfo();
		...
		VkSamplerCreateInfo sampler = vent::vkinitializer::samplerCreateInfo();
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); i++) {
			...
			*/
			{
				...
				VkRenderPassBeginInfo renderPassBeginInfo = vent::vkinitializer::renderPassBeginInfo();
				...
				VkViewport viewport = vent::vkinitializer::viewport((float)SHADOWMAP_DIM, (float)SHADOWMAP_DIM, 0.0f, 1.0f);
				vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

				VkRect2D scissor = vent::vkinitializer::rect2D(SHADOWMAP_DIM, SHADOWMAP_DIM, 0, 0);
			...
			*/

			{
				...
				VkViewport viewport = vent::vkinitializer::viewport((float)width, (float)height, 0.0f, 1.0f);
				vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

				VkRect2D scissor = vent::vkinitializer::rect2D(width, height, 0, 0);
				vkCmdSetScissor(drawCmdBuffers[i], 0, 1, &scissor);
	...
	void loadAssets()
	{
		...
		models.tree.loadFromFile(getAssetPath() + "scene/TreeSingle.gltf", vulkanDevice, queue, glTFLoadingFlags);
	}
	...
	void setupLayoutsAndDescriptors()
	{
		...
		*/
		std::vector<VkDescriptorPoolSize> poolSizes = {
			vent::vkinitializer::descriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 32),
			vent::vkinitializer::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 32)
		};
		VkDescriptorPoolCreateInfo descriptorPoolInfo =
			vent::vkinitializer::descriptorPoolCreateInfo(static_cast<uint32_t>(poolSizes.size()), poolSizes.data(), 4 + SHADOW_MAP_CASCADE_COUNT);
		...
		*/
		...
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
			vent::vkinitializer::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT, 0),
			vent::vkinitializer::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 1),
			vent::vkinitializer::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_FRAGMENT_BIT, 2),
		};
		VkDescriptorSetLayoutCreateInfo descriptorLayout =
			vent::vkinitializer::descriptorSetLayoutCreateInfo(setLayoutBindings);
		...
		*/
		...
		VkDescriptorImageInfo depthMapDescriptor =
			vent::vkinitializer::descriptorImageInfo(depth.sampler, depth.view, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL);

		VkDescriptorSetAllocateInfo allocInfo =
			vent::vkinitializer::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayouts.base, 1);

		// Scene rendering / debug display
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &descriptorSet));
		writeDescriptorSets = {
			vent::vkinitializer::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffers.VS.descriptor),
			vent::vkinitializer::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &depthMapDescriptor),
			vent::vkinitializer::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 2, &uniformBuffers.FS.descriptor),
		};
		...
		for (uint32_t i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
			...
			VkDescriptorImageInfo cascadeImageInfo = vent::vkinitializer::descriptorImageInfo(depth.sampler, depth.view, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL);
			writeDescriptorSets = {
				vent::vkinitializer::writeDescriptorSet(cascades[i].descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &depthPass.uniformBuffer.descriptor),
				vent::vkinitializer::writeDescriptorSet(cascades[i].descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &cascadeImageInfo)
			};
			vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
		}
		...
		*/
		...
		// Shared pipeline layout (scene and depth map debug display)
		{
			...
			VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo = vent::vkinitializer::pipelineLayoutCreateInfo(setLayouts.data(), static_cast<uint32_t>(setLayouts.size()));
	...
	{
		VkPipelineInputAssemblyStateCreateInfo inputAssemblyState = vent::vkinitializer::pipelineInputAssemblyStateCreateInfo(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0, VK_FALSE);
		VkPipelineRasterizationStateCreateInfo rasterizationState = vent::vkinitializer::pipelineRasterizationStateCreateInfo(VK_POLYGON_MODE_FILL, VK_CULL_MODE_BACK_BIT, VK_FRONT_FACE_CLOCKWISE, 0);
		VkPipelineColorBlendAttachmentState blendAttachmentState = vent::vkinitializer::pipelineColorBlendAttachmentState(0xf, VK_FALSE);
		VkPipelineColorBlendStateCreateInfo colorBlendState = vent::vkinitializer::pipelineColorBlendStateCreateInfo(1, &blendAttachmentState);
		VkPipelineDepthStencilStateCreateInfo depthStencilState = vent::vkinitializer::pipelineDepthStencilStateCreateInfo(VK_TRUE, VK_TRUE, VK_COMPARE_OP_LESS_OR_EQUAL);
		VkPipelineViewportStateCreateInfo viewportState = vent::vkinitializer::pipelineViewportStateCreateInfo(1, 1, 0);
		VkPipelineMultisampleStateCreateInfo multisampleState = vent::vkinitializer::pipelineMultisampleStateCreateInfo(VK_SAMPLE_COUNT_1_BIT, 0);
		std::vector<VkDynamicState> dynamicStateEnables = { VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR };
		VkPipelineDynamicStateCreateInfo dynamicState = vent::vkinitializer::pipelineDynamicStateCreateInfo(dynamicStateEnables);
		std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages;

		VkGraphicsPipelineCreateInfo pipelineCI = vent::vkinitializer::pipelineCreateInfo(pipelineLayout, renderPass, 0);
		...
		VkPipelineVertexInputStateCreateInfo emptyInputState = vent::vkinitializer::pipelineVertexInputStateCreateInfo();
		...
		*/
		...
		VkSpecializationMapEntry specializationMapEntry = vent::vkinitializer::specializationMapEntry(0, 0, sizeof(uint32_t));
		VkSpecializationInfo specializationInfo = vent::vkinitializer::specializationInfo(1, &specializationMapEntry, sizeof(uint32_t), &enablePCF);
