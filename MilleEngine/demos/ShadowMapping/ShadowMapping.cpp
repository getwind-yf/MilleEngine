...
public:
	...
	void prepareOffscreenFramebuffer()
	{
		...
		VkImageCreateInfo image = vent::vkinitializer::imageCreateInfo();
		...
		VkImageViewCreateInfo depthStencilView = vent::vkinitializer::imageViewCreateInfo();
		...
		VkFilter shadowmap_filter = vent::utils::formatIsFilterable(physicalDevice, offscreenDepthFormat, VK_IMAGE_TILING_OPTIMAL) ? VK_FILTER_LINEAR : VK_FILTER_NEAREST;
		...
		// Create frame buffer
		VkFramebufferCreateInfo fbufCreateInfo = vent::vkinitializer::framebufferCreateInfo();
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			*/
			{
				...
				viewport = vent::vkinitializer::viewport((float)offscreenPass.width, (float)offscreenPass.height, 0.0f, 1.0f);
				vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

				scissor = vent::vkinitializer::rect2D(offscreenPass.width, offscreenPass.height, 0, 0);
			...
			*/

			{
				...
				viewport = vent::vkinitializer::viewport((float)width, (float)height, 0.0f, 1.0f);
				vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

				scissor = vent::vkinitializer::rect2D(width, height, 0, 0);
				vkCmdSetScissor(drawCmdBuffers[i], 0, 1, &scissor);
	...
	void loadAssets()
	{
		...
		scenes.resize(2);
		scenes[0].loadFromFile(getAssetPath() + "scene/MillScene2.gltf", vulkanDevice, queue, glTFLoadingFlags);
		scenes[1].loadFromFile(getAssetPath() + "scene/Tree2.gltf", vulkanDevice, queue, glTFLoadingFlags);
		sceneNames = { "Mill scene", "Tree Scene" };
	}
	...
	void setupDescriptors()
	{
		...
		std::vector<VkDescriptorPoolSize> poolSizes = {
			vent::vkinitializer::descriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 3),
			vent::vkinitializer::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 3)
		};
		VkDescriptorPoolCreateInfo descriptorPoolInfo = vent::vkinitializer::descriptorPoolCreateInfo(poolSizes, 3);
		VK_CHECK_RESULT(vkCreateDescriptorPool(device, &descriptorPoolInfo, nullptr, &descriptorPool));

		// Layout
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
			// Binding 0 : Vertex shader uniform buffer
			vent::vkinitializer::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0),
		...
		VkDescriptorImageInfo shadowMapDescriptor =
			vent::vkinitializer::descriptorImageInfo(
		...
		writeDescriptorSets = {
			...
			vent::vkinitializer::writeDescriptorSet(descriptorSets.debug, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffers.scene.descriptor),
			// Binding 1 : Fragment shader texture sampler
			vent::vkinitializer::writeDescriptorSet(descriptorSets.debug, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &shadowMapDescriptor)
		};
		...
		writeDescriptorSets = {
			// Binding 0 : Vertex shader uniform buffer
			vent::vkinitializer::writeDescriptorSet(descriptorSets.offscreen, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffers.offscreen.descriptor),
		};
		vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
		...
		writeDescriptorSets = {
			// Binding 0 : Vertex shader uniform buffer
			vent::vkinitializer::writeDescriptorSet(descriptorSets.scene, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffers.scene.descriptor),
			// Binding 1 : Fragment shader shadow sampler
			vent::vkinitializer::writeDescriptorSet(descriptorSets.scene, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &shadowMapDescriptor)
		};
		vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
	}
	...
	void preparePipelines()
	{
		...
		// Pipelines
		VkPipelineInputAssemblyStateCreateInfo inputAssemblyStateCI = vent::vkinitializer::pipelineInputAssemblyStateCreateInfo(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0, VK_FALSE);
		VkPipelineRasterizationStateCreateInfo rasterizationStateCI = vent::vkinitializer::pipelineRasterizationStateCreateInfo(VK_POLYGON_MODE_FILL, VK_CULL_MODE_BACK_BIT, VK_FRONT_FACE_COUNTER_CLOCKWISE, 0);
		...
		VkPipelineColorBlendStateCreateInfo colorBlendStateCI = vent::vkinitializer::pipelineColorBlendStateCreateInfo(1, &blendAttachmentState);
		VkPipelineDepthStencilStateCreateInfo depthStencilStateCI = vent::vkinitializer::pipelineDepthStencilStateCreateInfo(VK_TRUE, VK_TRUE, VK_COMPARE_OP_LESS_OR_EQUAL);
		VkPipelineViewportStateCreateInfo viewportStateCI = vent::vkinitializer::pipelineViewportStateCreateInfo(1, 1, 0);
		VkPipelineMultisampleStateCreateInfo multisampleStateCI = vent::vkinitializer::pipelineMultisampleStateCreateInfo(VK_SAMPLE_COUNT_1_BIT, 0);
		std::vector<VkDynamicState> dynamicStateEnables = { VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR };
		VkPipelineDynamicStateCreateInfo dynamicStateCI = vent::vkinitializer::pipelineDynamicStateCreateInfo(dynamicStateEnables);
		...
		dynamicStateCI = vent::vkinitializer::pipelineDynamicStateCreateInfo(dynamicStateEnables);
